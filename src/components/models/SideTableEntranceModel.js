...
export default function SideTableEntranceModel(props) {
  const group = useRef();
  
  const { nodes, materials } = useGLTF("/assets/sidetableentrance.glb");
  const [sidetableblock] = useCylinder(() => ({ mass: 0, collisionResponse:true, position:[9.5,3.8,-1.1], args:[0.02,0.02,2]}))
  ...
  return (
    <group ref={group} {...props} dispose={null}>
      ...
      <mesh
        castShadow
        receiveShadow
        geometry={nodes.Object568.geometry}
...
useGLTF.preload("/assets/sidetableentrance.glb");